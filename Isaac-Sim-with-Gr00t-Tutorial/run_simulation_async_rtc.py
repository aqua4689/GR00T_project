...
RESULT_VIDEO_FILE = "./results/async_rtc.mp4"
    ...
    for ep in range(EPISODE_NUM):
        ...
        while frames < EACH_EPISODE_LEN:
            ...
            if (remaining_in_chunk <= LEFT_ACTION_THRESHOLD) and (not started_async_this_chunk) and (not fetcher.in_flight()) and (staged_resp is None):
                ...
                current_actions_progressed = {}
                for action_name, current_action in current_actions.items():
                    # Ver. 1
                    # current_action_np = np.array(current_actions[action_name])
                    # current_action_progressed = np.zeros_like(current_action_np)
                    # current_action_progressed[:remaining_in_chunk, :] = current_action_np[-remaining_in_chunk:, :]
                    # current_actions_progressed[action_name] = current_action_progressed.tolist()
                    
                    # Ver. 2
                    current_actions_progressed[action_name] = np.array(current_action)[-remaining_in_chunk:, :].tolist()

                print("[simulation] remaining in chunk:", remaining_in_chunk)
                print("[simulation] actions:", np.array(current_actions['action.left_arm']).shape, np.array(current_actions['action.left_arm'])[:, 0])
                print()

                # payload["current_actions"] = current_actions    # add current actions to send them to the inference server (should slice and zero-pad later)
                payload["current_actions"] = current_actions_progressed
                fetcher.start(
                    payload,
                    H_at_start=t_cursor,
                    global_timestep_at_start=global_action_timestep # Pass global step
